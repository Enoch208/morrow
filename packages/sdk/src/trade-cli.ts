...
import { prepareTradeStep } from "./trade-steps.ts";
import { tradeChain, tradeSigner } from "./trade-roles.ts";
import { isProbeStep } from "./trade-probe.ts";
import { runProbe } from "./trade-probe-run.ts";
...
try {
  ...
  if (isProbeStep(step)) {
    await runProbe(step, wallet, rpc, records, options, broadcast);
  } else {
