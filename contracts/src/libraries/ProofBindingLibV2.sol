...
import {AttestcoinGateV2} from "./AttestcoinGateV2.sol";
import {SaleTermsLib} from "./SaleTermsLib.sol";

library ProofBindingLibV2 {
    ...
    function reservation(
        ...
        address sourceVault
    ) internal returns (bytes32 eventKey) {
        EvmV1Decoder.LogEntry memory entry;
        (entry, eventKey) = AttestcoinGateV2.authenticate(proof, logIndex, sourceVault, RESERVED);
    ...
    function outcome(
        ...
        EvmV1Decoder.LogEntry memory entry;
        (entry, eventKey) = AttestcoinGateV2.authenticate(proof, logIndex, sourceVault, assigned ? ASSIGNED : CANCELLED);
        bindIdentity(entry, terms);
