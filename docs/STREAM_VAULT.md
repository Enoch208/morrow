...
- **Browser flow:** the dashboard Trade page is wired to the first market only. Stream sales run through the journaled CLI, whose seller assignment gate checks chain id, runtime code of the market and vault, market immutables, exact BOUND terms, liability coverage, finalized funding, the open source round and the deadline.
