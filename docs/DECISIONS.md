...
For a current-block replay, obtain a fresh continuity witness from the official proof service while requiring the recorded source transaction hash, chain key, source height, transaction bytes and Merkle proof to remain unchanged. Re-encode both the native and application calls from that checked envelope. Error expectations are state-specific: the old-round replay establishes `SaleIdMismatch` at its recorded BOUND block, while the same proof returns `SaleNotBound` after the later sale becomes terminal. Both are retained rather than relabeling the latest-state guard as historical round-binding evidence.
